...
module.exports = (webserver, controller) => {
  ...
  webserver.get("/twitter-webhooks", (req, res) => {
    /* Handle crc_token. */
    let crc_token = req.param("crc_token");

    if (crc_token) {
      console.log({ crc_token });
      let response_token = `sha256=${crypto
        .createHmac("sha256", twitter_config.consumer_secret)
        .update(crc_token)
        .digest("base64")}`;
  ...
  webserver.post("/twitter-webhooks", (req, res) => {
    ...
    if (req.body.direct_message_events) {
      let message = req.body.direct_message_events[0],
      ...
      switch (message.type) {
        case "message_create":
          let message_id = message.id,
            ...
            quick_reply_response =
              message.message_create.message_data.quick_reply_response;
          /*
        ...
        */
          ...
          if (sender_screen_name !== process.env.twitter_screen_name) {
            ...
            console.log(
              `new direct message from ${sender_name} (@${sender_screen_name}): > ${message_text}`
            );
